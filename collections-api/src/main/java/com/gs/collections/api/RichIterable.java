        ...
        extends InternalIterable<T>
{
     ...
     * causes predicate to evaluate to true is included in the new projection.
     * <p/>
     * <pre>e.g.
    ...
    /**
     * Same as the select method with one parameter but uses the specified target collection
     * <p/>
     * <pre>e.g.
    ...
    /**
     * For each element of the receiver, predicate is evaluated with the element as the parameter. Each element which
     * causes predicate to evaluate to false is included in the new projection.
     * <p/>
     * <pre>e.g.
    ...
    /**
     * Same as the reject method with one parameter but uses the specified target collection
     * <p/>
     * <pre>e.g.
     * return people.reject(new Predicate&lt;Person&gt;()
     * {
     ...
     *     }
     * }, Integer.valueOf(100), new ArrayList<Person>());
     * </pre>
     *
    ...
    /**
     * Same as the collectIf method with two parameters but uses the specified target collection.
     *
     * @param predicate a {@link Predicate} to use as the select criteria
     * @param function  a {@link Function} to use as the collect transformation function
     ...
     * projection have been used as arguments. That is, there may be elements of the receiver that are never used as
     ...
     * result as the parameters. The first evaluation of function is performed with injectedValue as the first
    ...
    /**
     * Converts the collection to a mutable MutableList implementation and sorts it using the natural order of the
     * elements.
     *
     * @since 1.0
     */
    ...
    /**
     * Converts the collection to a mutable MutableList implementation and sorts it based on the natural order of the
    ...
    /**
     * Converts the collection to a mutable MutableSet implementation.
     *
     * @since 1.0
     */
    MutableSet<T> toSet();
    ...
    /**
     * Converts the collection to a mutable MutableMap implementation using the specified key and value functions.
     *
     * @since 1.0
     */
    ...
    /**
     * Converts the collection to a mutable MutableSortedMap implementation using the specified key and value functions
     ...
     * <p/>
     * <p>Suppose <tt>x</tt> is a collection known to contain only strings. The following code can be used to dump the
    ...
    /**
     * Returns the minimum elements out of this container based on the natural order of the attribute returned by
     *
     * @throws NoSuchElementException if the RichIterable is empty
     * @since 1.0
     */
    ...
    /**
     * Returns the maximum elements out of this container based on the natural order of the attribute returned by
     *
     * @throws NoSuchElementException if the RichIterable is empty
     * @since 1.0
     */
    ...
    /**
     * For each element of the receiver, transformer is evaluated with the element as the parameter. The results of
     * these evaluations are collected into a new multimap, where the transformed value is the key and the original
     * values are the values. The elements are traversed in the same order as they would be if the message #do: had been
