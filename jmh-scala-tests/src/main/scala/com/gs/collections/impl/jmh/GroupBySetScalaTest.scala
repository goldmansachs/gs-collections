...
object GroupBySetScalaTest
{
    ...
    private val integers = new HashSet[Int]() ++ (1 to SIZE)
    ...
    def groupBy_2_keys_parallel_lazy_scala()
    {
        Assert.assertEquals(2, this.integers.par.groupBy((each: Int) => each % 2 == 0).size)
    }

    def groupBy_100_keys_parallel_lazy_scala()
    {
        Assert.assertEquals(100, this.integers.par.groupBy((each: Int) => each % 100).size)
    }

    def groupBy_10000_keys_parallel_lazy_scala()
    {
        Assert.assertEquals(10000, this.integers.par.groupBy((each: Int) => each % 10000).size)
    }
}
