...
public interface MutableCollectionTestCase extends CollectionTestCase, RichIterableTestCase
{
    ...
    default void MutableCollection_iterationOrder()
    {
        ...
        this.getInstanceUnderTest().injectIntoWith(0, new Function3<Integer, Integer, Integer, Integer>()
        {
            @Override
            public Integer value(Integer argument1, Integer argument2, Integer argument3)
            {
                injectIntoWithIterationOrder.add(argument2);
                return 0;
            }
        }, 0);
        assertEquals(this.newMutableForFilter(4, 4, 4, 4, 3, 3, 3, 2, 2, 1), injectIntoWithIterationOrder);

        // TODO removeIf and removeIfWith
    }
    ...
    {
        MutableCollection<Integer> collection = this.newWith(5, 5, 4, 4, 3, 3, 2, 2, 1, 1);
        collection.removeIf((Predicate<? super Integer>) each -> each % 2 == 0);
        assertEquals(this.getExpectedFiltered(5, 5, 3, 3, 1, 1), collection);
    }
    ...
    default void MutableCollection_removeIfWith()
    {
        ...
        assertEquals(this.getExpectedFiltered(4, 4, 2, 2), collection);
    }
    ...
    default void MutableCollection_injectIntoWith()
    {
        ...
        assertEquals(Integer.valueOf(81), collection.injectIntoWith(1, new Function3<Integer, Integer, Integer, Integer>()
        {
            @Override
            public Integer value(Integer argument1, Integer argument2, Integer argument3)
            {
